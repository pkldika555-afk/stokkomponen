    ...
    <div class="max-w-2xl mx-auto px-6 py-8">
        ...
        @if($errors->any())
            <div class="bg-rose-500/10 border border-rose-500/30 rounded-xl px-4 py-3 mb-5 flex gap-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-rose-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <form action="{{ route('mutasi.store') }}" method="POST"
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div>
                    ...
                    <select id="id_komponen" name="id_komponen"
                        ...
                        @foreach($komponen as $k)
                            <option value="{{ $k->id }}"
                                ...
                                data-stok-minimal="{{ $k->stok_minimal }}"
                    ...
                    <div id="stok-info" class="hidden mt-2 bg-gray-800/60 border border-gray-700/60 rounded-lg px-3.5 py-2.5 flex items-center justify-between">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select id="jenis" name="jenis"
                            ...
                            <option value="internal"       {{ old('jenis') == 'internal'       ? 'selected' : '' }}>📤 Pemakaian Internal</option>
                            <option value="retur"          {{ old('jenis') == 'retur'          ? 'selected' : '' }}>↩️ Retur</option>
                            <option value="repair_kembali" {{ old('jenis') == 'repair_kembali' ? 'selected' : '' }}>🔧 Repair Kembali</option>
                        </select>
                        @error('jenis')
                            <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="number" id="jumlah" name="jumlah"
                        value="{{ old('jumlah') }}" min="1" placeholder="0"
