...
<div class="min-h-screen bg-gray-950 text-gray-100 font-sans">
    <div class="max-w-2xl mx-auto px-6 py-10">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-500 text-sm mt-1">Perbarui data komponen <span class="text-indigo-400 font-medium">{{ $komponen->nama_komponen }}</span></p>
        </div>
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
                ...
                <p class="text-2xl font-bold {{ ($komponen->stok ?? 0) > ($komponen->stok_minimum ?? 0) ? 'text-emerald-400' : 'text-rose-400' }}">
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
                ...
                <p class="text-xs text-gray-600 mt-0.5">{{ $komponen->updated_at ? $komponen->updated_at->format('H:i') : '' }}</p>
            </div>
            <div class="bg-gray-900 border border-gray-800 rounded-xl p-4 text-center">
                <p class="text-xs text-gray-500 mb-1">ID Komponen</p>
                <code class="text-sm font-mono text-indigo-400">#{{ str_pad($komponen->id, 4, '0', STR_PAD_LEFT) }}</code>
                <p class="text-xs text-gray-600 mt-0.5">sistem</p>
            </div>
        </div>

        <form action="{{ route('komponen.update', $komponen->id) }}" method="POST" class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="px-6 py-6 space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="nama_komponen" name="nama_komponen" value="{{ old('nama_komponen', $komponen->nama_komponen) }}" placeholder="Contoh: Resistor 10K Ohm" class="w-full bg-gray-800 border {{ $errors->has('nama_komponen') ? 'border-rose-500' : 'border-gray-700' }} text-gray-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition placeholder-gray-600">
                    @error('nama_komponen')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="kode" name="kode" value="{{ old('kode', $komponen->kode) }}" placeholder="Contoh: KMP-001" class="w-full bg-gray-800 border {{ $errors->has('kode') ? 'border-rose-500' : 'border-gray-700' }} text-gray-100 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition placeholder-gray-600 font-mono">
                        @error('kode')
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" id="stok_minimum" name="stok_minimum" value="{{ old('stok_minimum', $komponen->stok_minimum) }}" min="0"class="w-full bg-gray-800 border {{ $errors->has('stok_minimum') ? 'border-rose-500' : 'border-gray-700' }} text-gray-100 rounded-xl px-4 py-3 pr-16 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition font-mono">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1.5">
                        Stok Saat Ini
                        <span class="ml-1 text-xs text-gray-500 font-normal">(hanya bisa diubah melalui mutasi barang)</span>
                    </label>
                    <div class="relative">
                        <input type="number" value="{{ $komponen->stok ?? 0 }}" disabled class="w-full bg-gray-800/50 border border-gray-700 text-gray-500 rounded-xl px-4 py-3 pr-16 text-sm cursor-not-allowed font-mono">
                    ...
                    <p class="mt-1.5 text-xs text-gray-600">
                        ...
                        <a href="{{ route('mutasi.create') }}" class="text-indigo-400 hover:text-indigo-300 underline underline-offset-2">Mutasi Barang</a>
                    </p>
                </div>
            ...
            <div class="px-6 py-4 border-t border-gray-800 bg-gray-900/50 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <form action="{{ route('komponen.destroy', $komponen->id) }}" method="POST"
                          onsubmit="return confirm('Yakin ingin menghapus komponen {{ addslashes($komponen->nama_komponen) }}? Tindakan ini tidak bisa dibatalkan.')">
                        @csrf @method('DELETE')
                        <button type="submit"
                            class="px-5 py-2.5 rounded-xl border border-rose-800/50 text-rose-500 hover:bg-rose-500/10 hover:border-rose-500/50 text-sm font-medium transition-colors">
                            Hapus
                        </button>
                    </form>
                </div>
